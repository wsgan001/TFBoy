...
def computeYbyX(x):
    ...
    # )
    return noise + 400 * np.sin(x)  # + 2 * x * x  # 用它模拟要生成的曲线
...
def two_layers():  # 用到2层网络
    ...
    w = weight_variable([hidden_dim, 1])  # 400 * 1
    b = bias_variable([hidden_dim, 1])  # 400 * 1

    w2 = weight_variable([1, hidden_dim])  # 1*400
    b2 = bias_variable([1])
    ...
    # w是400*1的矩阵，x是1*401的矩阵，填充xTrain训练数据，b是400*1的增量
    ...
    # y = tf.matmul(x, w3) + b3  # 这个单层模型形状比较接近，需要修改  (1*401) * (401*401)
    ...
    # print "w3:", sess.run(w4)
    # print "b3:", sess.run(b4)
    # hidden = tf.Print(hidden, [hidden], message="Hidden is:")
    # b = tf.add(hidden, hidden).eval(session=sess)
...
def classify():
    ...
    for i in xrange(200):
        ...
        circle_1_x = math.sin(theta) * (r ** 0.5) + 1
        circle_1_y = math.cos(theta) * (r ** 0.5) + 1
